# React Admin Dashboard: React.js, Vite, Material UI - Dark & Light Mode

React Admin Dashboard is a modern and responsive administration panel built with React.js, Vite, and Material UI. It provides powerful data visualization, user management, analytics tracking, and interactive dashboard components with seamless Dark and Light Mode support for an enhanced user experience.


## Features

- **Dashboard Page:** An overview of key metrics and insights.
- **Manage Team Page:** Efficiently handle team members with MUI DataGrid.
- **Contacts Information:** Keep track of contacts effortlessly with a DataGrid.
- **Invoices Balances Page:** Manage invoice balances conveniently using MUI DataGrid.
- **Profile Form Page:** Create new user profiles seamlessly with Formik and Yup integration.
- **Calendar Page:** Interactive calendar functionality powered by FullCalendar.
- **FAQ Page:** Provide answers to common queries using MUI Accordion.
- **Charts Pages:** Visualize data effectively with various chart types including Bar, Pie, Line, Geography, and Stream.

## Demo

Explore the live demo of React Admin Dashboard [react-dashboard-tawny-one.vercel.app].

## Get Started

1. Clone the repository: `git clone https://github.com/tirumanimukesh-dev/React_Dashboard.git`
2. Install dependencies: `npm install` or `yarn install`
3. Start the development server: `npm run dev` or `yarn dev`   

## Project Links

| Resource | Link |
|-----------|------|
| GitHub Repository | https://github.com/tirumanimukesh-dev/React_Dashboard |
| Live Demo | https://react-dashboard-tawny-one.vercel.app/ |
